...
class ShadedReliefCreator(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber('contrastmin', 'Contrast Min', type=QgsProcessingParameterNumber.Integer, minValue=-255, maxValue=255, defaultValue=-20))
    ...
    def processAlgorithm(self, parameters, context: QgsProcessingContext, model_feedback):
        ...
        for idx in range(2):
          if idx == 0:
              expr = QgsExpression('315 - (@angle_between_light_sources / 2)')
          else:
              expr =  QgsExpression('@angle_between_light_sources / 2')

          # Hillshade
          alg_params = {
              'ALTITUDE': 45,
              'AZIMUTH': expr.evaluate(),
              'BAND': 1,
              'COMBINED': False,
              'COMPUTE_EDGES': False,
              'EXTRA': '',
          ...
          hillshadeMin = outputs[f'HillshadeStats{idx}']['MIN']
          hillshadeMax = outputs[f'HillshadeStats{idx}']['MAX']
          
          contrastMin = parameters['contrastmin']
          contrastMax = parameters['contrastmax']
